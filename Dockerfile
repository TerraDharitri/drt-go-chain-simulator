...
RUN cp /go/pkg/mod/github.com/TerraDharitri/$(cat /TerraDharitri /go.sum | grep drt-go-chain-vm | sort -n | tail -n -1 | awk -F '/' '{print$3}' | sed 's/ /@/g')/wasmer2/libvmoaeccapi.so /lib_amd64/
...
RUN cp /go/pkg/mod/github.com/TerraDharitri/$(cat /TerraDharitri /go.sum | grep drt-go-chain-vm | sort -n | tail -n -1 | awk -F '/' '{print$3}' | sed 's/ /@/g')/wasmer2/libvmoaeccapi_arm.so /lib_arm64/
